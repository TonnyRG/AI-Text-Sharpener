...
**文字／公式判定：** 新扫描结合文字内容、OCR 置信度及分式／上下标的图像结构判断。可疑区域放大后重新识别，同时保留整页 OCR 候选；普通文字与公式分别试排，在同一块原图上比较轮廓、位置和墨迹量。普通文字只有达到质量门槛、明显胜出且无复杂结构时才自动恢复。两者接近或拟合都不佳时保留原图，加入「集中复核」，可确认或保留后移出待办。公式仍需核对后启用。该流程在本机运行，耗时随可疑区域数量增加；已有工作区需重新识别才应用新判定，不自动覆盖编辑。
