# 文字重绘 · AI Text Sharpener Studio

在本机识别 AI 图片中的文字，匹配接近原图的字体、字重、字号、字距和位置，擦除旧字后以真实字体轮廓重绘。保留原图设计，重点改善文字的清晰度与还原程度。

**v0.4.0：Linux 本地应用首版，已在 Fedora 44 / Python 3.12 上验证。** 无需 API Key，OCR 与字体匹配均在本机进行。首次安装需要联网下载依赖；处理图片不调用云服务。

## 当前电脑直接使用

在应用菜单搜索 **文字重绘**，或运行：

```bash
./scripts/launch-studio.sh
```

启动后访问 <http://127.0.0.1:8766>。界面内有可直接体验的中英文测试示例。

1. 导入 PNG、JPEG 等图片，或每页为一张完整图片的 PPTX。图片等比适配页面时，每侧不超过 1 个原图像素的边缘误差可直接导入，保留原始像素；明显留边、裁剪、旋转及混合图文页面仍需先导出为 PNG。
2. 点击 **批量识别…**，选择 **重新识别全部页面** 或 **仅处理未完成页面**；只处理一页时点击 **识别本页**。
3. 切换 **原图／重绘** 或 **按住看原图** 检查效果；点击文字区域可改文字、字体及字重、字号、字距、颜色和坐标，也可拖动微调。
4. 右侧按内容、字体与样式、文字位置与大小、原字擦除范围分组，可点击标题折叠；字体候选与详细说明按需展开。修改后会自动更新预览，也可点击 **刷新预览**。预览复用导出的矢量渲染逻辑，不会清空撤销记录。选择 **按当前字体匹配**，可固定字体重新估计字号、字距与位置。
5. **Ctrl+Z** 撤销，**Ctrl+Shift+Z / Ctrl+Y** 重做。每页保留最近 80 步；切换页和导出不会清空，切换项目或刷新窗口后清空。
6. 点击 **导出…**，先选择 **当前页 / 全部页面**，再选择 **PPTX / SVG / PNG**。全部页面的 SVG / PNG 按页码打包为 ZIP，当前页直接下载单文件；PPTX 包含所选范围。可选择保存文件夹（浏览目录或填写完整路径）并修改文件名，自动补齐扩展名；同名文件自动编号，不覆盖已有文件。导出完成后显示实际路径，并记住上次保存目录。编辑数据 JSON 备份也保存到所选目录。项目自动保存在本机，关闭网页后可继续编辑。

**界面分区：** 左侧「页面 / 本页文字」分别使用完整列表空间；中间上方放选择移动、添加文字与原图对比，下方放文字框、吸附和缩放；右侧「文字编辑」调整内容、字体和位置，「原图修复」调整原字擦除与背景。切回文字编辑会退出橙色擦除框模式；集中复核的「定位文字」自动打开文字列表与编辑面板。页签支持左右方向键、Home / End，切换本身不改动项目。参照产品和取舍见 [界面参考记录](docs/ui-reference-notes.md)。

## 输出格式

| 格式 | 文字形式 | 用途 |
| --- | --- | --- |
| SVG | 真实字体的矢量路径 | 放大文字仍清晰，无需接收方安装字体 |
| PNG | 原图分辨率的无损位图 | 通用预览与图片工作流 |
| PPTX | SVG 图片 + 无损 PNG 兼容后备图 | 支持 SVG 的演示软件显示矢量文字；不支持时显示 PNG |

**集中复核：** 顶部「集中复核」按页汇总拟合较弱、识别不确定、异常字距、窄体／斜体、文字相碰及超出页面的区域。提示不代表必然错误；点击「定位文字」可跳转到重绘视图中的文字框；「保留原图」保留此框内的原始像素及识别参数，开启「重绘此区域」即可恢复重绘。支持本页未锁定待复核区域批量保留，单处和批量保留均可 Ctrl+Z 撤销。旧工作区无需重新 OCR 即可复核。

**多色文字：** 新的识别／字体匹配会尝试恢复同一行中的强调色。已有结果可点击「恢复多色…」，选择当前页或全部页面，不重新识别文字或改变排版。跳过锁定、保留原图和手动配色区域；拟合较弱、文字内容已变化、仅有明暗差异或颜色证据不足时保留现有配色。右侧「字体与样式 → 多色文字」可逐片段修正颜色；也可先在文字内容中选中字，再「给选中文字着色」。改动文字内容后清除旧分色，移动和缩放保留颜色；修改整行颜色或点击「整行统一颜色」可退出多色。颜色恢复及手动配色支持按页撤销，并在预览、SVG、PNG 和 PPTX 使用同一套轮廓。自动恢复当前针对纯色片段，标点边界、复杂渐变和特殊字形仍需核对。

**工作区管理：** 左侧「工作区 → 管理」可删除工作区。删除后进入应用回收站，可随时恢复；不会删除最初导入的图片或 PPTX。删除当前工作区会打开剩余工作区，没有剩余时回到导入界面。识别、匹配或导出进行中不能删除工作区。

**直接在画布编辑：** 已处理的页面打开后显示重绘结果。选中绿色文字框，拖框内移动文字，拖四角缩放，拖上方圆点旋转（接近 0°、±45°、±90°、±135°、180° 或其他重绘文字角度的 3° 内自动吸附；显示角度和紫色参考线；Shift 按 15° 旋转，Alt 临时关闭吸附）；字号、字距与笔画随缩放变化，原图擦除范围保持在原处。拖动时直接显示文字变化，松开后自动保存，并用导出同样的渲染器更新预览。普通文字和已启用的公式均支持；锁定或保留原图的区域不参与拖动。移动时靠近页面中心／边缘、其他重绘文字框的边缘／中心，会显示紫色参考线并自动吸附；吸附距离随缩放换算，参考线不进入导出。取消「对齐吸附」或按住 Alt 可自由移动，方向键仍可逐像素微调。一次拖动可用 Ctrl+Z 撤销。

**字号与字距：** 自动匹配、手动输入和拖动缩放统一使用整数（负字距同样按绝对值四舍五入）。打开旧工作区时自动取整并刷新预览，无需确认；再次保存或导出时使用整数结果。字体匹配会比较附近的整数字号和字距，不再返回小数候选。

**框选多个区域：** 在画布或周围空白处按住左键拖动，选中完全包含在框内的文字；反向拖动同样有效，旋转文字按实际四角判断。Shift / Ctrl 点击可增减选择，点击空白或按 Esc 取消。选中后可整体移动、方向键微调或 Delete 删除；锁定区域不参与移动或删除，保留原图的区域不参与移动。框选后点击右侧「保留选中原图」，可批量恢复选中文字区域的原始像素，预览和导出同步生效；锁定区域会跳过并提示。整体移动、删除和批量保留均可一次撤销。

**对齐与分布：** 多选后在右侧选择左对齐、水平居中、右对齐、顶端对齐、垂直居中、底端对齐，或横向／纵向等间距。对齐以选中区域的可见边界为准；等间距至少选三处，固定首尾区域。支持旋转文字与公式，跳过锁定和保留原图区域，每次操作可一次撤销。原图视图下需先切回重绘。

**自动对齐校正：** 新识别页面在字体拟合后，依据原图实际字迹寻找至少三处相邻文字的共同边缘或中心线。仅校正高置信水平文字的轻微偏移（最多 4 px），再次对照原图，避免新碰撞；缩进、旋转文字、公式及手动／锁定区域不自动整理。不自动等间距或重新排版。已有结果不会自行改变；可使用多选按钮，或重新识别页面来应用自动校正。

**多选批量样式：** 框选或 Shift 点击选中多个区域后，右侧显示「批量字体与样式」。可以统一字体／字重、字号、字距、笔画加粗与颜色；属性不同时显示「多种」，只修改操作的那一项。字号与字距自动取整；每次批量操作可一次撤销／重做。跳过锁定及保留原图区域，公式可批量改字号、笔画与颜色，但保持数学字体与排版。批量换字体先验证整批能否渲染，缺字时保留原样并提示。统一颜色会改为整行单色，修改其他属性保留原多色。

**公式误判恢复：** 高置信中英混排且无明显数学运算符的普通行优先保留为文字。公式区域也保存原 OCR 文本；选择「普通文字」会自动恢复内容并匹配字体。旧公式没有保存文字时，会在该区域重新做普通 OCR；识别失败则保留原区域并提示调整范围。

**文字／公式判定：** 新扫描结合文字内容、OCR 置信度及分式／上下标的图像结构判断。可疑区域放大后重新识别，同时保留整页 OCR 候选；普通文字与公式分别试排，在同一块原图上比较轮廓、位置和墨迹量。普通文字只有达到质量门槛、明显胜出且无复杂结构时才自动恢复。两者接近或拟合都不佳时保留原图，加入「集中复核」，可保留原图后移出待办。公式仍需核对后启用。该流程在本机运行，耗时随可疑区域数量增加；已有工作区需重新识别才应用新判定，不自动覆盖编辑。

**倾斜文字：** 识别记录原字四角与方向，校正方向后匹配字体，再旋回原角度；支持倾斜和整行旋转 90° / 180° 的单行文字。右侧「文字位置与大小 → 旋转角度」可精确输入，画布圆点可直接拖动，预览与 SVG / PNG / PPTX 同步。移动、缩放、旋转保持原图擦除位置；旋转框仍可对齐吸附、撤销。手动框选倾斜文字时，在「原图修复 → 原图文字角度」设原字方向后再匹配。旧工作区需重新识别相应页面以获得方向信息，或手动设置原字角度。弧形文字、透视变形及逐字直立的传统竖排不属于整行旋转，仍需拆分处理。

按住 **按住看原图** 临时隐藏重绘与框线，松开返回，也支持聚焦按钮后按住空格或回车。**原图** 模式仅供查看；修改原字的识别/擦除范围时，切换右侧 **原图修复 → 原字擦除范围**，再操作橙色框。

**PPTX 中的文字是矢量轮廓，不是 PowerPoint 原生文本框。** 可在 Studio 中修改文字后重新导出。SVG/PPTX 背景仍是原始位图；矢量化的是重新绘制的文字。不同演示软件对 SVG 的支持需在目标软件中确认。

## 如何提高接近程度

- 自动匹配会实际渲染一组本机字体，与原字形比较，给出前五个候选。分数只用于比较图形接近程度，不是 OCR 正确率或质量保证。
- 匹配和擦字会排除框顶／框底与本行明显分离的邻行残片，保留本行的点、重音和标点；字重评分同时比较轮廓与墨迹量，减少普通文字被误加粗。单字符会比较水平与 OCR 角度，只有倾斜候选明显更接近原图时才采用；圆形编号框的边角背景不计入字形。现有区域需重新匹配或重新识别，已有手动编辑不会自动重算。
- 中文和英文分别选择候选字体；初选字体后，会补充比较前三个候选字体家族的全部已安装直立字重（包括 Regular、Medium），避免候选数量限制漏掉合适字重。安装与原图接近的字体通常比反复调整字号更有效。
- **笔画加粗 / px** 用于同一字体内的小幅增厚，0 表示原始字重，步长 0.05。优先选对真实字重，再尝试 0.1–0.5 px 微调；需要变细时改选较轻字重。增厚使用矢量描边，不改变字间的排版进位，预览、SVG 和 PPTX 使用同一几何轮廓。
- OCR 漏字时可手动画框；同一行混合字体、字号、颜色时请拆成多个区域。
- **纯色 / 渐变补全**适合平面幻灯片；**纹理局部修复**适合较复杂背景；**保留背景，仅叠字**可用于已有干净底图。
- 拟合和擦字都可能失误。低置信度、明显倾斜或无合适字体的区域默认保留原图；不满意的区域也可取消“重绘此区域”。
- 原字擦除框与新文字位置独立：拖动新字不会留下原字。橙色虚线表示擦除范围，绿色框表示新文字。
- 点击 **调整 OCR 框** 后，可拖动橙色框的八个边角手柄改变大小，拖动内部移动范围；也可输入框 X、Y、宽、高。调框不会移动或缩放重绘文字，需要时再匹配字体。清空文字内容会擦除原字；删除区域或关闭重绘则保留原图。
- 识别时使用识别器的逐字位置校验整行框，再按完整笔画扩展边界，减少将邻近图形当成文字的情况。不按图形颜色或形状作特殊判断；文字与图形相连或重叠、字符定位不可靠时仍需手动调整。

## 已知边界

当前重点是水平单行的中英文常规排版。弯曲字、透视字、艺术变形、描边发光、阴影、复杂纹理和严重乱码，尚不能保证忠实恢复。AI 生成的字形也可能不对应任何现成字体。工具提供候选与手动校正，不承诺所有图片一键完美。

PPTX 导入严格支持单张图片铺满页面且无裁剪、旋转的幻灯片。包含原生文字、图表、组合形状的 PPTX，请先在原演示软件中导出页面图片再导入。多图片可一次导入，也支持批量识别。

**批量处理规则：**「识别 → 全部页面」提供两个选项：

- **重新识别全部页面**：重新 OCR 并匹配字体，替换已有文字区域、样式、手动修改和锁定区域。开始前在项目的 `backups/before-rescan-*.json` 中保存配置备份，原始图片不变。
- **仅处理未完成页面**：保留已完成结果，继续尚未识别或上次重扫未完成的页面。

每完成一页自动保存。取消或单页失败时，该页旧结果仍保留，后续可选「仅处理未完成页面」续做。导出按所选范围包含当前页或全部页面，未识别的页面保留原图。

## 在其他 Linux 电脑安装

需要 Python 3.11–3.13（当前验证 3.12）、fontconfig、可用的中英文字体与浏览器。项目中不分发商业字体。字体可安装至 `~/.local/share/fonts`，运行 `fc-cache -f` 后重启应用。

```bash
python3.12 -m venv .venv
.venv/bin/python -m pip install -e .
.venv/bin/python -m ai_text_sharpener.studio
```

在有 systemd 用户服务的桌面 Linux 上安装应用入口：

```bash
.venv/bin/python scripts/install-linux.py
./scripts/launch-studio.sh
```

服务按需启动，未设置登录自动启动。停止服务：

```bash
systemctl --user stop ai-text-sharpener
```

不使用 systemd 时，可直接运行 `ai-text-sharpener-studio --port 8766`。服务器仅监听 `127.0.0.1`。项目默认位于 `~/.local/share/ai-text-sharpener/projects/`，遵循 `XDG_DATA_HOME`，也可通过 `--data-dir` 指定目录。备份请复制整个项目文件夹，包含原图与 `project.json`；单独 JSON 不包含原图。项目修改源码路径后需重新运行安装脚本。

当前 Linux 环境的依赖快照见 `requirements-linux.lock.txt`。这是 Python 3.12 / Linux 环境快照，不是 Windows 通用锁文件。

## 开发与验证

渲染测试需要 fontconfig、拉丁字体及覆盖中文的字体。Ubuntu / GitHub Actions 使用 `fontconfig fonts-liberation fonts-noto-cjk`；可先运行 `sudo apt-get install -y fontconfig fonts-liberation fonts-noto-cjk`。测试不依赖个人安装的商业字体。

```bash
.venv/bin/python -m pip install -e '.[dev]'
.venv/bin/python -m pytest -q
```

Studio 核心模块：`typography.py`（真实字形与布局）、`fidelity.py`（匹配与背景修复）、`studio_project.py`（持久化及矢量导出）、`studio.py`（本地服务）、`web/`（界面）。验证记录见 [docs/studio-validation.md](docs/studio-validation.md)。

旧版 PaddleOCR 命令行和编辑器仍保留。需额外安装 `pip install -e '.[paddle]'`，使用方式见 [旧版工作流](docs/legacy-workflow.md)。旧工作流与 Studio 的项目格式、导出能力不同；旧文档中的原生文本编辑指旧编辑器内部，不表示导出的 PPTX 可原生编辑文字。

## License

Apache 2.0，见 [LICENSE](LICENSE)。

### 公式识别与二维重绘（可选，本机 CPU）

在已有 Python 3.11 / 3.12 Studio 环境中运行：

```bash
.venv/bin/python scripts/install-formulas.py
systemctl --user restart ai-text-sharpener.service
```

安装器下载约 260 MB 模型和 MathJax，另外创建 `.venv-formula`；不修改普通文字识别环境。需 Node.js 18+（可用 `--node /路径/node` 指定；本机 Codex 自带 Node 也可自动发现）。组件安装后，处理图片时不联网、不上传图片、不需要 API Key。模型、运行时配置和来源校验位于 `~/.local/share/ai-text-sharpener/formula/`。

识别当前页 / 整个 PPT 时，先检测完整公式区域，合并其中的 OCR 碎片，再生成 LaTeX 候选。公式默认完整保留原图；在右侧核对 LaTeX 后，勾选「重绘此区域」即可预览和导出。也可手动画框，将「区域类型」改为「公式」，调整框覆盖整个公式后点击「重新识别公式」。修改 LaTeX 自动更新预览，「匹配公式位置」重新拟合尺寸、位置和颜色。

公式使用独立数学字体生成 SVG 路径，支持分数、上下标、积分、根号、矩阵和分段结构；可调整字号、笔画粗细、颜色和位置。当前不能像普通文字一样匹配任意系统字体。公式 OCR 仍可能认错字符或结构，相似度不是正确率，不会据此自动启用公式。保留原图的公式依然是位图；启用重绘的公式才是矢量轮廓。模型本身未经过本项目微调。

自动文字拟合若得分低于 0.35 或明显超出原字范围，会保留原图；该限制不阻止手动改字、移动或放大。旧版项目导出也会过滤失败的自动拟合。已有项目需重新扫描才能生成新的完整公式区域。

实现来源：[Pix2Text MFD](https://huggingface.co/breezedeus/pix2text-mfd-1.5)、[RapidLaTeXOCR](https://github.com/RapidAI/RapidLaTeXOCR)、[MathJax](https://github.com/mathjax/MathJax-src)。安装时固定版本与模型哈希；模型不随源码打包。MFD 模型卡标注 MIT，ONNX 元数据同时带有 Ultralytics AGPL-3.0 信息，二者不一致已记录，重新分发模型前需另行核实。MathJax 与 RapidLaTeXOCR 的许可文件保留在安装目录。
