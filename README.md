...
4. 右侧按内容、字体与样式、文字位置与大小、原字擦除范围分组，可点击标题折叠；字体候选与详细说明按需展开。修改后会自动更新预览，也可点击 **更新预览**。预览复用导出的矢量渲染逻辑，不会清空撤销记录。选择 **按当前字体匹配**，可固定字体重新估计字号、字距与位置。
...
**直接在画布编辑：** 已处理的页面打开后显示重绘结果。选中绿色文字框，拖框内移动文字，拖四角等比缩放；字号、字距与笔画一起变化，原图擦除范围保持在原处。拖动时直接显示文字变化，松开后自动保存，并用导出同样的渲染器更新预览。普通文字和已启用的公式均支持；锁定或保留原图的区域不参与拖动。移动时靠近页面中心／边缘、其他重绘文字框的边缘／中心，会显示紫色参考线并自动吸附；吸附距离随缩放换算，参考线不进入导出。取消「对齐吸附」或按住 Alt 可自由移动，方向键仍可逐像素微调。一次拖动可用 Ctrl+Z 撤销。
